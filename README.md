...
2. Publicly Available Code
   <br/>URL: https://github.com/chenchongthu/DeepCoNN/tree/master


## Models
1. `DeepCoNN:` A model that jointly models users and items from textual review data to predict ratings.

2. `Modified version of the DeepCoNN:` In the original DeepCoNN model, when predicting the user's rating, we assumed a situation where the user does not have review data for the item in question. **Therefore, the rating prediction for the item is designed to be based on the review data and ratings of other users for other items, as well as the review data and ratings of the user for other items, using the relationship between them and employing Euclidean Distance for prediction.**
